...
> 🧑‍🎓 = à produire par vous · 💻 = fourni. Le formateur annonce le rythme, les pauses et l'ouverture éventuelle du bonus.
...
**Sortie de départ attendue dans le zip apprenant : `10 failed · 1 passed · 1 skipped`.** C'est normal :
les dix échecs matérialisent les fonctions que vous allez compléter et le test ignoré attend l'artefact modèle
produit plus tard. Ne modifiez jamais `tests/test_drift.py` pour « faire du vert ». À la fin du TP, la même
commande doit afficher **`12 passed`**.
...
Sortie de départ attendue : **`10 failed · 1 passed · 1 skipped`** ; preuve finale : **`12 passed`**.
...
## Étape 1 — (Re)poser la baseline : entraîner et geler le modèle 💻
...
## Étape 3 — Semaine 1 : première ronde de surveillance 🧑‍🎓
...
## Étape 4 — Semaine 2 : quelque chose a changé 🧑‍🎓
...
## Étape 5 — Semaine 3 : la ronde tranquille… ? 🧑‍🎓
...
## Étape 6 — Les labels arrivent : l'heure de vérité 🧑‍🎓
...
- **Q16 (synthèse module 31).** Renseignez les trois fiches ci-dessous, puis nommez le type de dérive
  ...
  Chaque fiche garde les mêmes cinq rubriques :
  ...
  - **Semaine 1**
    ...
    - taux d'alerte :
    - type de dérive :
    - deux preuves : une distributionnelle et une métier.
  - **Semaine 2**
    - PSI des entrées :
    - rappel :
    - taux d'alerte :
...
## Étape 7 — Synthèse : votre drift spec 🧑‍🎓
...
## Étape 8 — BONUS : le même audit avec Evidently (sur consigne du formateur) 💻
...
>
> **Suite au module 32 :** ce calcul devient un rapport produit à chaque run dans le flow Prefect,
> après `predict`, avec alerte anti-bruit (seuil et cooldown) et événements en base. Vos choix de
> drift spec — features, tests, seuils, fenêtre et fréquence — restent inchangés : l'outil change,
> pas la spec.
